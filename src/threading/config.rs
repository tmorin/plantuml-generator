...
/// Thread count can be configured via the `PLANTUML_GEN_THREADS` environment
/// variable.
///
/// # Examples
///
...
impl Config {
    ...
    ///
    /// Reads the `PLANTUML_GEN_THREADS` environment variable. If not set or
    /// invalid, falls back to the default (CPU core count).
    ///
    /// # Environment Variables
    ///
    /// * `PLANTUML_GEN_THREADS` - Number of threads (1-256)
    ///
    /// # Examples
    ///
    /// ```ignore
    /// use crate::threading::Config;
    ///
    /// // With environment variable set: PLANTUML_GEN_THREADS=8
    /// let config = Config::from_env();
    /// assert_eq!(config.thread_count(), 8);
    /// ```
    pub fn from_env() -> Self {
        const ENV_VAR: &str = "PLANTUML_GEN_THREADS";
...
mod tests {
    ...
    fn test_from_env_not_set() {
        env::remove_var("PLANTUML_GEN_THREADS");
        let config = Config::from_env();
    ...
    fn test_from_env_valid() {
        env::set_var("PLANTUML_GEN_THREADS", "16");
        let config = Config::from_env();
    ...
    fn test_from_env_invalid_falls_back() {
        env::set_var("PLANTUML_GEN_THREADS", "invalid");
        let config = Config::from_env();
    ...
    fn test_from_env_out_of_range() {
        env::set_var("PLANTUML_GEN_THREADS", "300");
        let config = Config::from_env();
        assert_eq!(config.thread_count(), 4); // Falls back to default
        env::remove_var("PLANTUML_GEN_THREADS");
    }
